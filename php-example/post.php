...
// $app_id=$data["app_id"];
// $publiser_id=$data["publiser_id"];
// $bidfloor=$data["bid_floor"];
// $w=$data["ad_width"];
// $h=$data["ad_height"];
// $os=$data["os"];
// $Osv=$data["os_ver"];
...
// $app_cat=$data["app_cat"];
// $btype=$data["btype"];
// $mimes=$data["mimes"];
// $badv=$data["badv"];
// $bcat=$data["bcat"];
  // $data = array($days,$hours,$exchange_id,$app_id,$publiser_id,$bidfloor,$w,$h,$os,$Osv,$model,$connectiontype,$country,$ua,$carrier,$js,$user,$carriername,$app_cat,$btype,$mimes,$badv,$bcat);
 // $data = implode("\t",$data);
 // print_r($data);
  ...
  echo "<br>";
// exec R script
exec("Rscript /usr/share/nginx/html/wr_predict_online.R $data 2>&1",$output,$return_val);
print_r($output);
echo "<br />";
print_r($return_val);
echo "<br />";
exec("Rscript /usr/share/nginx/html/wp_predict_online.R $data 2>&1",$output1,$return_val1);
print_r($output1);
echo "<br />";
print_r($return_val1);
echo "<br />";
